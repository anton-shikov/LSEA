...
def make_clumps(input_dict,file_name, plink_dir, bfile):
    with open(os.getcwd()+ "/"+file_name.split('.')[0]+'_for_plink.tsv','w', newline='') as csvfile:
        ...
        snp_cmd = subprocess.call('{0}/plink --bfile {1} --clump {2} --clump-field P --clump-p1 5e-08 --clump-r2 0.1 --clump-snp-field SNP --clump-kb 500 --out {3}'.format(plink_dir,plink_dir +'/'+ bfile,os.getcwd()+ "/"+file_name.split('.')[0]+'_for_plink.tsv',os.getcwd()+ "/"+file_name.split('.')[0]+'.clumped'),shell=True)

def annotate_vcf(file_name, snpeff, genome):

    snp_cmd = subprocess.call('java -Xmx4g -jar {0} -v {1} {2} > {3}'.format(snpeff+'/snpEff.jar',genome,os.getcwd() + "/"+file_name,os.getcwd()+"/"+file_name.split('.')[0]+"_annotated.vcf"),shell=True)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='LSEA')
    parser.add_argument('-af', help='Please enter input file', metavar='File',
                        type=str, required=True)
    parser.add_argument('-vf', help='Enter vcf file', metavar='File',
                        type=str, required=False)
    parser.add_argument('-pl', help='Enter plink file', metavar='File',
                        type=str, required=False)
    parser.add_argument('-cf', help='Full Manhattan cluster file', metavar='File',
                        type=str, required=False)
    parser.add_argument('-fa', help='Full association file', metavar='File',
                        type=str, required=False)
    parser.add_argument('-va', help='Full VCF annotation file', metavar='File',
                        type=str, required=False)
    parser.add_argument('--precompiled', '-p',action='store_true',help='use precompiled loci')
    parser.add_argument('-sn', help='Please enter path to SNPeff', metavar='File',
                        type=str, required=True)
    parser.add_argument('-g', help='Please enter genome for SNPeff', metavar='Str',
                        type=str, required=False, default='GRCh37.75')
    parser.add_argument('-pld', help='Please enter path to plink', metavar='Dir',
                        type=str, required=True)
    parser.add_argument('-bf', help='Please enter bfile for plink', metavar='Str',
                        type=str, required=False, default='1000G_20101123_v3_GIANT_chr1_23_minimacnamesifnotRS_CEU_MAF0.01')
    parser.add_argument('-cld', help='Please enter directory with clumped files', metavar='Dir',
                        type=str, required=False, default='')
    parser.add_argument('--create_universe', '-cu',action='store_true',help='create universe for SNP data')
